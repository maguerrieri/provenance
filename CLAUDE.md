...
- `researcher.md` says to cite the version the claim is about and to set `date` to its effective
  date or version ("operative …", "as amended by …", "current through …"), and both agents
  say what `date` holds for each kind of page. What is specific to one jurisdiction's hosts
  (which they are, and where an earlier version of a section is) goes in that list's notes
  (`ca-notes.md`, after "What comes in series here"), and a test holds each list's notes to
  naming every `legal_text` host the list has.
...
- **What counts as left out** is decided by the figure's own grouping. The left-out rows are
  ...
  it as a row with no readable amount, never as `$0.00`, and lists its share first (and then
  does not call the shares it cuts off "smaller"). A ranking that counts nothing names one only
  beside a left-out gift with an amount. Alone, kept or withdrawn, such a gift leaves the ranking
  naming nobody, so its filing is not one a person need open, and the miss stays one to retry.
  Beside one, it can overturn the leader a person restores from the other filing. The totals'
