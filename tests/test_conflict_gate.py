...
def test_a_unit_is_a_whole_word():
    ...
    # Every form the first-letter read caught by accident, which a whole word must list.
    assert money_values("$5MM, $6 mil, $2.5 bil, $3 mln, $4 bln, $8 millions") == {
        5e6, 6e6, 2.5e9, 3e6, 4e9, 8e6}
    # A hyphen after the unit leaves it one: a range, a compound.
    assert money_values("valued at $1.5M-$2M, a $7M-a-year deal") == {1.5e6, 2e6, 7e6}
    # A letter followed by a digit is not a unit.
    assert money_values("$10 m2") == {10.0}
