...
UNREADABLE_ENTRIES = [
    ...
    # it could never match a cited source, so it would read as lapsed, silently judging nothing
...
def test_vg_judgments_reads_a_miscased_shard_as_build_does(tmp_path):
    ...
    if folds_case:
        ...
        # nothing reads it here, and a source id alone never makes it q1's
        assert "under an id no claim has (Q1.json)" in out, out

    before = _shard_files(cand)
    (cand / "judgments" / "Q1.json").rename(cand / "judgments" / "tmp.json")
...
QUERY_DEFINITIONS = {
    "calaccess.contributor_total": (6, "f37ab574741a"),
    "calaccess.filer_total": (6, "f1743dc041fa"),
    "calaccess.top_contributor": (6, "f1a079c22860"),
    ...
    "calaccess.ie_total": (2, "04a49a997c31"),
}
