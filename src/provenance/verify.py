...
def secondary_host(src: Source, rules: dict[str, tuple[str, ...]] | None = None) -> bool:
    ...
    Which hosts issue records is decided by `rules`: the project's source lists and the hosts it
    names itself (`primary_hosts`, through `Project.rules()`). A host neither names reads as a copy.
